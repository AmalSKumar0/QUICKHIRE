...
if (isset($_POST['delete_job'])) {
    $job_id = $_POST['job_id'];
    $query = "DELETE FROM job_postings WHERE job_id = ? AND firm_id = ?"; // Changed table name to job_postings
    $stmt = $conn->prepare($query);
    $stmt->bind_param("is", $job_id, $_SESSION['firm_id']); // Assuming job_id is an integer and firm_id is a string
    if ($stmt->execute()) {
        echo "<p>Job post deleted successfully.</p>";
...
// Query to fetch job posts created by the logged-in firm user
$query = "SELECT job_id, job_title, job_description, job_date, job_timing, job_status, wage, created_at, job_location 
          FROM job_postings
          WHERE firm_id = ? 
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<p class='card__text'><strong>Time:</strong> " . htmlspecialchars($row['job_timing']) . "</p>"; // Changed from job_time to job_timing
        ...
        echo '<button type="submit" name="delete_job" class="but">Delete</button>';
        echo "</form>";
        echo '</div>';
        echo '</div>'; // Close card
    }
