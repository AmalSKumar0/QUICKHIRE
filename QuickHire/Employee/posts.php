...
if (isset($_POST['delete_job'])) {
    ...
    if($stmt->execute()){
        if ($_POST['type'] == 'firm') {  // Use '==' to compare values
            // Correct DELETE query syntax
            $sql = $conn->prepare("DELETE FROM job_postings WHERE job_id = ?");
            $sql->bind_param("i", $_POST['id']);
            if (!$sql->execute()) {
                echo "<script>console.log('NOT DELETED FROM JOB POSTING');</script>";  // Fixed 'console.log'
            }
        } else {
            // Correct DELETE query for elderly_jobs
            $sql = $conn->prepare("DELETE FROM elderly_jobs WHERE job_id = ?");
            $sql->bind_param("i", $_POST['id']);
            if (!$sql->execute()) {  // Execute once and check for failure
                echo "<script>console.log('NOT DELETED FROM ELDERLY JOBS');</script>";  // Fixed 'console.log'
            }
        }
        
        echo "<script> alert('Thank you for your service'); </script>";
...
$query = "SELECT jr.request_id, jr.job_type, jr.job_id, jr.employee_id, jr.request_status, jr.request_date,
          ...
          WHERE jr.employee_id = ? AND jr.request_status = 'accepted'
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<p class='card__text'><strong>Status:</strong> " . htmlspecialchars($row['request_status']) . "</p>";
         ...
         echo "<input type='hidden' name='type' value='".$row['job_type']."'>";
         echo '<button type="submit" name="delete_job" value="'.$row['request_id'].'" class="but">Completed</button>';
         echo "</form>";
...
function confirmDelete() {
    return confirm("Did you completed the job?");
}
</script>
<br>